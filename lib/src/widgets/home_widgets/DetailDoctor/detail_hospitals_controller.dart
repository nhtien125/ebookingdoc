...
class DetailHospitalsController extends GetxController {
  String id = '';
  ...
  void bookAppointment() {
    if (selectedTimeIndex.value != -1 && doctor.value != null) {
      final selectedDate = availableDates[selectedDateIndex.value];
      final selectedTime =
          doctor.value!.availableSlots[selectedTimeIndex.value];

      print(
          'Booking appointment for ${doctor.value!.name} on ${DateFormat('dd/MM/yyyy').format(selectedDate)} at $selectedTime');
